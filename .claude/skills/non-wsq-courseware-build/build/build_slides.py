...
def tile_grid(title,items,kicker=None,cols=2,size=15,icons=None,accent=BLUE,_start=0):
    ...
    bd=Inches(0.6)
    # Auto-fit: shrink this grid's text so even the longest string tile fits its
    # card height (uniform per page). Tuple tiles keep the passed size.
    if items and all(not isinstance(it,tuple) for it in items):
        ch_in=ch/914400.0; cw_in=cw/914400.0
        availh=max(ch_in-0.26,0.4); availw=max(cw_in-1.4,1.0)
        longest=max(items,key=lambda s:len(str(s)))
        size=min(size,_fit_size(longest,availw,availh,size,11))
    for i,it in enumerate(items):
...
def trainer_slide(kicker,name,role,rows,initials,accent=BLUE):
    ...
    footer(s); return s
def _fit_size(text, width_in, avail_in, hi, lo):
    """Largest font (hi..lo) at which `text`, wrapped to width_in inches, fits
    within avail_in inches of height. Estimates ~0.5em average glyph width and
    1.2x line spacing; conservative so text never clips into the region below."""
    import math
    chars=max(len(str(text)),1)
    for fs in range(hi, lo-1, -1):
        cpl=max(int(width_in*72/(0.5*fs)),1)
        lines=math.ceil(chars/cpl)
        if lines*fs*1.2/72.0 <= avail_in:
            return fs
    return lo
def activity_overview(tag,title,desc,build,services,kicker):
    ...
    # Size is derived from the text length so the wrapped block stays within the
    # ~1.6in gap above the panel (fs <= sqrt(k/chars)); clamped to a sane range.
    dsize = _fit_size(desc, 11.7, 1.62, 21, 12)   # gap from 2.55in to the panel at 4.3in
    ...
    # Auto-fit the build line so it stays within its row above the Tools line at 5.6in.
    bsize = _fit_size(build, 11.0, 0.66, 18, 12)
    txt(s,Inches(1.1),Inches(4.9),Inches(11),Inches(0.6),[[(build,bsize,_ink(),True)]])
...
def step_slide(kicker,act_title,n,total,text,cmd=""):
    ...
    # prompt panel appears only for a real prompt.
    has_cmd = bool(cmd) and not cmd.lstrip().startswith("#")
    if has_cmd:
        # Instruction sits above the prompt panel; auto-fit so long steps never clip.
        isize=_fit_size(text,10.1,1.45,23,14)
        txt(s,Inches(2.55),Inches(2.35),Inches(10.1),Inches(1.5),[[(text,isize,_ink(),False)]],anchor=MSO_ANCHOR.TOP)
        # Prompt panel: multi-paragraph, font and height fitted to the prompt text.
        segs=str(cmd).split("\n"); inner=9.5
        def _plines(fs):
            cpl=max(int(inner*72/(0.5*fs)),1)
            return sum(max(1,math.ceil(len(seg)/cpl)) for seg in segs)
        pfs=10
        for cand in (13,12,11,10):
            pfs=cand
            if _plines(cand)*cand*1.2/72.0 <= 2.30: break
        boxh=min(2.55, _plines(pfs)*pfs*1.2/72.0 + 0.34); y0=4.12
        txt(s,Inches(2.55),Inches(3.82),Inches(10),Inches(0.3),[[("PROMPT — paste into ChatGPT, Claude or Gemini",12,_acc(BLUE),True)]])
        if THEME["dark"]:
            rect(s,Inches(2.55),Inches(y0),Inches(10.1),Inches(boxh),RGBColor(0x10,0x14,0x1E),line=DK_FAINT)
        else:
            rect(s,Inches(2.55),Inches(y0),Inches(10.1),Inches(boxh),RGBColor(0x0B,0x12,0x20))
        runs=[[((seg if seg.strip() else " "),pfs,RGBColor(0x9C,0xDC,0xFE),False)] for seg in segs]
        txt(s,Inches(2.8),Inches(y0+0.13),Inches(inner),Inches(boxh-0.26),runs,anchor=MSO_ANCHOR.TOP,space=2)
    else:
        # No prompt — center the (possibly long) instruction in the body area.
        isize=_fit_size(text,10.1,2.5,26,16)
        txt(s,Inches(2.55),Inches(2.4),Inches(10.1),Inches(2.6),[[(text,isize,_ink(),False)]],anchor=MSO_ANCHOR.MIDDLE)
    footer(s); return s
